...
function saveDatabase() {
  fs.writeFile('database', yaml.safeDump(database), 'utf8', (err) => {
    if (err) throw err;
  });
}
...
function loadDatabase() {
  try {
    // I used readFileSync as this should only occur when the server first
    // starts. It seemed reasonable to block on i/o in this case.
    const data = fs.readFileSync('database', 'utf8');
    database = yaml.safeLoad(data);
  }
  catch (e) {
    if (e.code === 'ENOENT') {
      // First server run. File does not exist yet.
    }
    else {
...
function updateComment(url, request) {
  ...
  if (!requestComment || !request.body || !requestComment.body || !id) {
    response.status = 400; // Bad request
  }
...
function deleteComment(url, request) {
  ...
  if (!id) {
    ...
    // Get array of user comments
    let userComments = database.users[database.comments[id].username].commentIds;

    // Remove comment that matches the ID
    userComments.splice(userComments.indexOf(Number.parseInt(id)), 1);

    // Get array or article comments
    let articleComments = database.articles[database.comments[id].articleId].commentIds;

    // Remove the comment that matches the ID
    articleComments.splice(articleComments.indexOf(Number.parseInt(id)), 1);
...
function upvoteComment(url, request) {
  ...
  if (!id || !database.comments[id] || !request || !request.body ||
    !request.body.username || !database.users[request.body.username]) {
    response.status = 400; // Bad request
  }
  else {
    const user = request.body.username;

    if (database.comments[id].upvotedBy.indexOf(user) < 0) {
      // Has not yet been upvoted
      database.comments[id].upvotedBy.push(user);

      const downvotedByIndex = database.comments[id].downvotedBy.indexOf(user);

      if (downvotedByIndex >= 0) {
        // Has been downvoted, remove from downvotedBy array.
        database.comments[id].downvotedBy.splice(downvotedByIndex, 1);
      }
    }
    response.status = 200; // OK
    response.body = {
...
function downvoteComment(url, request) {
  ...
  if (!id || !database.comments[id] || !request || !request.body ||
    ...
    if (database.comments[id].downvotedBy.indexOf(user) < 0) {
      // Has not yet been downvoted
      database.comments[id].downvotedBy.push(user);

      const upvotedByIndex = database.comments[id].upvotedBy.indexOf(user);

      if (upvotedByIndex >= 0) {
        // Has been upwnvoted, remove from upwnvotedBy array.
        database.comments[id].upvotedBy.splice(upvotedByIndex, 1);
      }
    }
    response.status = 200; // OK
    response.body = {
